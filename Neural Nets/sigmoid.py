...
for i in xrange(1000):
	...
	for i2 in range(len(h)):
		count = count + 1
		if(i2 == 0):
			hidden_layer.append(1 / (1.0 + np.exp(-(np.dot(X, w_array[i2]) + b_array[i2]))))
			
		else:
			hidden_layer.append(1 / (1.0 + np.exp(-(np.dot(hidden_layer[i2-1], w_array[i2]) + b_array[i2]))))
	...
	dhidden_array[l3-1] = np.multiply(dhidden_array[l3-1],np.multiply(hidden_layer[l3-1],(1-hidden_layer[l3-1])))
	...
	for h1 in range(0,l3-2):
		...
		dhidden_array[l3-2-h1] = np.multiply(dhidden_array[l3-2-h1],np.multiply(hidden_layer[l3-2-h1],(1-hidden_layer[l3-2-h1])))
	...
	dhidden_array[0] = np.multiply(dhidden_array[0],np.multiply(hidden_layer[0],(1-hidden_layer[0])))
 ...
 #  		w_array[jk] += -step_size * dw_array[len(dw_array)-(1+jk)]
	...
	# dw_array.append(np.dot(hidden_layer[l3-1].T, dscores))
